...
export function HeroSection({ t, scrollToSection, cvUrl, staticCvUrl, shouldReduceMotion }: HeroSectionProps) {
  ...
  const heroSignalCards = [
    {
      ...
      className: "border-accent-green/20 bg-accent-green/[0.075] text-green-text",
    },
    {
      key: "server",
      icon: <ServerIcon />,
      ...
      className: "border-accent-cyan/20 bg-accent-cyan/[0.075] text-cyan-text",
    },
    {
      key: "ai",
      icon: <CloudIcon />,
  ...
  return (
    <section id="home" className="relative isolate overflow-hidden px-4 pt-20 text-foreground sm:px-6 sm:pt-24 lg:pt-28">
      <div className="absolute inset-0 bg-background" aria-hidden="true" />
      <div
        className="absolute inset-0 bg-[linear-gradient(115deg,rgba(34,211,238,0.12)_0%,transparent_24%,rgba(47,226,138,0.12)_48%,transparent_70%,rgba(16,185,129,0.1)_100%)]"
        aria-hidden="true"
      />
      <div
        ...
        aria-hidden="true"
      />
      <div className="absolute inset-x-0 top-0 h-px bg-gradient-to-r from-transparent via-accent-cyan/60 to-transparent" aria-hidden="true" />
      ...
      <div className="relative mx-auto grid min-h-[calc(100svh-5rem)] max-w-7xl items-center gap-10 pb-14 lg:grid-cols-[0.92fr_1.08fr] lg:gap-12 xl:gap-16">
        ...
        >
          <div className="flex flex-wrap items-center gap-3">
            ...
            <motion.p
              ...
              className="rounded-lg border border-line bg-hover/[0.045] px-3 py-2 text-[11px] font-semibold uppercase tracking-[0.18em] text-secondary backdrop-blur-xl"
            >
              {t.hero.role}
            </motion.p>
          </div>
          ...
          <motion.h1
            ...
            className="mt-6 max-w-[10ch] text-[3.25rem] font-black leading-[0.9] tracking-normal text-foreground min-[390px]:text-[3.75rem] sm:max-w-none sm:text-7xl lg:text-[5.75rem] xl:text-[6.6rem]"
          >
            {heroPrimaryName}
            {heroAccentName && (
              <span className="block bg-gradient-to-r from-accent-cyan via-accent-blue to-accent-green bg-clip-text text-transparent">
                {heroAccentName}
              </span>
            )}
          </motion.h1>
          ...
          <motion.p
            ...
            className="mt-6 max-w-2xl text-base leading-7 text-secondary sm:text-lg lg:text-xl lg:leading-8"
          >
            {t.hero.description}
          </motion.p>
          ...
          >
            {t.hero.specialties.map((specialty, index) => (
              <span
                key={specialty}
                className={`rounded-lg border px-3.5 py-2 text-xs font-bold shadow-lg shadow-elevation/10 backdrop-blur-xl sm:text-sm ${
          ...
          >
            <button
              ...
              className="inline-flex min-h-14 items-center justify-center gap-2 rounded-lg bg-accent-green px-6 py-3.5 text-sm font-black text-inverse shadow-2xl shadow-accent-cyan/20 transition-all duration-300 hover:-translate-y-0.5 hover:bg-accent-cyan hover:shadow-accent-cyan/25 focus:outline-none focus:ring-2 focus:ring-accent-cyan focus:ring-offset-2 focus:ring-offset-background"
            >
              <MailIcon />
              {t.hero.contact}
            </button>
          ...
          >
            {t.hero.stats.map((stat) => (
              <div key={stat.label} className="border-line p-4 odd:border-r sm:border-r sm:last:border-r-0">
                <p className="text-2xl font-black tracking-normal text-foreground sm:text-3xl">{stat.value}</p>
                <p className="mt-1 text-[10px] font-bold uppercase tracking-[0.16em] text-muted sm:text-[11px]">{stat.label}</p>
              </div>
            ))}
          </motion.div>
        </motion.div>
        ...
        >
          <div className="relative">
            <div className="absolute -inset-px rounded-2xl bg-gradient-to-br from-accent-cyan/40 via-white/10 to-accent-green/35 opacity-70 blur-sm" aria-hidden="true" />
            <div className="relative overflow-hidden rounded-2xl border border-line bg-background/72 shadow-2xl shadow-elevation/50 backdrop-blur-2xl">
              <div className="flex items-center justify-between border-b border-line bg-hover/[0.035] px-4 py-3 sm:px-5">
                <div className="flex items-center gap-2" aria-hidden="true">
                  <span className="h-2.5 w-2.5 rounded-full bg-rose-400" />
                  <span className="h-2.5 w-2.5 rounded-full bg-amber-300" />
                  <span className="h-2.5 w-2.5 rounded-full bg-accent-green" />
                </div>
                <span className="font-mono text-xs text-muted">{t.hero.visual.file}</span>
              </div>
                ...
                <div className="relative grid gap-4">
                  <div className="grid gap-4 lg:grid-cols-[1.15fr_0.85fr]">
                    <motion.div
                      ...
                      className="relative min-h-72 overflow-hidden rounded-xl border border-accent-cyan/15 bg-background/78 p-4 shadow-xl shadow-accent-cyan/20"
                    >
                      <div className="flex items-start justify-between gap-4">
                        <div>
                          <p className="font-mono text-[11px] font-bold uppercase tracking-[0.18em] text-cyan-text">
                      ...
                      <div className="relative mt-6 h-44 overflow-hidden rounded-xl border border-line bg-surface/65">
                        ...
                        <div className="absolute inset-x-6 top-7 flex justify-between">
                          {t.hero.visual.nodes.slice(0, 2).map((node, index) => (
                            <motion.div
                              ...
                              className="grid h-14 w-14 place-items-center rounded-xl border border-accent-cyan/25 bg-background/90 font-mono text-xs font-black text-cyan-text shadow-lg shadow-accent-cyan/10 backdrop-blur-xl"
                            >
                              {node}
                            </motion.div>
                          ))}
                        ...
                        <div className="absolute inset-x-9 bottom-7 flex justify-between">
                          {t.hero.visual.nodes.slice(2).map((node, index) => (
                            <motion.div
                              ...
                              className="grid h-14 w-14 place-items-center rounded-xl border border-accent-green/25 bg-background/90 font-mono text-xs font-black text-green-text shadow-lg shadow-accent-green/10 backdrop-blur-xl"
                            >
                              {node}
                            </motion.div>
                          ))}
                        ...
                        <motion.div
                          ...
                          className="absolute left-1/2 top-1/2 grid h-20 w-20 -translate-x-1/2 -translate-y-1/2 place-items-center rounded-xl border border-accent-green/25 bg-accent-green/[0.1] text-green-text shadow-2xl shadow-accent-green/10 backdrop-blur-xl"
                        >
                          <ShieldIcon />
                        </motion.div>
                      </div>
                    ...
                    <div className="grid gap-4">
                      {heroSignalCards.map((card, index) => (
                        <motion.div
                          ...
                          className={`rounded-xl border p-4 shadow-xl shadow-elevation/20 backdrop-blur-xl ${card.className}`}
                        >
                          <div className="flex items-start gap-3">
                            <div className="grid h-11 w-11 flex-shrink-0 place-items-center rounded-lg border border-line bg-background/50 text-foreground">
                              {card.icon}
                            </div>
                            <div className="min-w-0">
                              <p className="text-xs font-bold uppercase tracking-[0.14em] text-secondary">{card.title}</p>
                  ...
                  <div className="grid gap-3 sm:grid-cols-4">
                    {primarySpecialties.map((specialty, index) => (
                      <div
                        key={specialty}
                        className="rounded-xl border border-line bg-hover/[0.045] p-3 shadow-lg shadow-elevation/10 backdrop-blur-xl"
                      >
                        <p className="font-mono text-[10px] font-bold uppercase tracking-[0.18em] text-subtle">
                          {String(index + 1).padStart(2, "0")}
                        </p>
                        <p className="mt-2 text-sm font-bold leading-5 text-foreground">{specialty}</p>
                      </div>
                    ))}
                  </div>
                </div>
      ...
      >
        <span className="block h-8 w-4 rounded-full border border-line/70 p-1">
          <span className="mx-auto block h-1.5 w-1 rounded-full bg-accent-cyan" />
        </span>
      </motion.button>
    </section>
  );
