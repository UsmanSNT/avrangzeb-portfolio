  ...
  const handleReaction = async (id: number, reaction: 'like' | 'dislike') => {
    ...
    try {
      ...
      if (result.success && result.data) {
        ...
        // Update viewing quote if open
        if (viewingQuote?.id === id) {
          const updatedQuote = bookQuotes.find(q => q.id === id);
          if (updatedQuote) {
            setViewingQuote({
              ...updatedQuote,
              likes: result.data.likes || updatedQuote.likes,
              dislikes: result.data.dislikes || updatedQuote.dislikes,
              userReaction: result.data.userReaction || null,
            });
          }
        }
